...
export interface StockRow {
  ...
  // ── แพคมีของไม่ตรงจำนวน (v1.23) — เจอตอนเปิดแพคใช้ ซึ่งเลยขั้นตอนยืนยันรับของไปแล้ว ──
  // เก็บเป็นส่วนต่างหน่วยย่อย (ถ้วย = ชิ้น · อื่น ๆ = กรัม) · +2 = แพคมีเกินมา 2 · -1 = ขาดไป 1
  // แยกจากช่อง "รับเข้า" เพราะต้องรู้ทีหลังว่าของที่เพิ่มมาเป็นของที่สั่ง หรือส่วนเกินจากแพคไม่ครบ
  packAdjust?: number;
}
...
export interface Session {
  ...
  branchScope: BranchScope;
  exp: number; // epoch ms
  // วันที่ (เวลาไทย) ที่ล็อกอิน — ข้ามวันแล้วต้องใส่ PIN ใหม่เสมอ แม้ session ยังไม่หมดอายุ
  // เพื่อบังคับให้ทุกคนเริ่มวันด้วยการล็อกอิน แล้วลงเวลาเข้างานจริง (แพรสั่ง 2026-07-28)
  day?: string;
...
export type AdminFlagReason =
  ...
  // แก้ยอด (ช่องไหนก็ได้) ของ "วันนี้" หลังจากที่นับ+ยืนยันคงเหลือไปแล้วรอบหนึ่ง (แพรขอ 2026-08-05)
  // คนละเคสกับ stock_backdated_edit ซึ่งเป็นการย้อนไปแก้วันเก่า — อันนี้คือวันเดียวกันแต่แก้ซ้ำ
  | "stock_same_day_edit"
  ...
  | "receipt_vs_manual"
  // แพคถ้วยมีของไม่ตรงจำนวนที่ระบุ (เจอตอนเปิดใช้) — ของส่วนเกินเก็บเป็นของร้าน แต่แอดมินต้องรู้
  | "cup_pack_mismatch"
